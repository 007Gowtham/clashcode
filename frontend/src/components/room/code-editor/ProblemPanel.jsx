...
function SubmissionRow({ sub }) {
  ...
  const date = sub.createdAt ? new Date(sub.createdAt) : null;
  ...
  const passed = (sub.testResults || []).filter(r => r.pass).length;
    ...
    <div className="border border-gray-100 rounded-lg overflow-hidden">
      ...
      {expanded && (
        <div className="border-t border-gray-100 bg-gray-50 px-3 py-2">
          {sub.output && (
            <p className="text-xs text-gray-600 mb-2">{sub.output}</p>
          )}
          <pre className="text-[11px] font-mono text-gray-700 bg-white border border-gray-200 rounded p-2 overflow-x-auto max-h-32 whitespace-pre-wrap">
            {sub.code}
          </pre>
        </div>
      )}
...
function SubmissionsTab({ submissions, loading, questionId }) {
  const filtered = (submissions || []).filter(s =>
    !questionId || String(s.questionId) === String(questionId) || s.questionId?._id === questionId
  );
...
export default function ProblemPanel({
  ...
  return (
    <div className="flex flex-col h-full bg-white overflow-hidden font-[family-name:var(--font-inter)]" style={style}>
                ...
                {leaderboard.map((team, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-lg shadow-sm hover:border-slate-300 transition-colors">
                    <div className="flex items-center gap-3">
                      <div className="flex items-center justify-center w-7 h-7 rounded-sm bg-slate-900 text-white font-bold text-xs shrink-0">
                        {team.rank || (idx + 1)}
                      </div>
                      <div className="flex flex-col">
                        <span className="font-bold text-slate-900 text-[13px]">{team.teamName}</span>
                    ...
                    <div className="flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded border border-emerald-200 font-extrabold text-[13px]">
