...
import { Plus, Users, CheckCircle2, LogOut, Shield } from 'lucide-react';

function Badge({ children, className }) {
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-semibold font-[family-name:var(--font-inter)] ${className}`}
    >
      {children}
    </span>
  );
...
export default function TeamCard({ team, onJoinTeam, onLeaveTeam, onReady, myTeamId }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between w-full mb-4">
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 tracking-tight font-[family-name:var(--font-mono)] leading-tight">
                {team.name}
              </h3>
        ...
        <div className="flex flex-col items-end gap-1 shrink-0">
          <span className="px-2 py-0.5 rounded text-[10px] font-medium bg-slate-100 text-slate-500 font-[family-name:var(--font-inter)]">
            {isPublic ? 'Public' : 'Private'}
          </span>
          {allReady && (
            <span className="flex items-center gap-1 px-2 py-0.5 rounded bg-emerald-50 border border-emerald-200 text-emerald-700 text-[10px] font-bold font-[family-name:var(--font-inter)]">
      ...
      <div className="mb-4 w-full">
        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 font-[family-name:var(--font-inter)]">
        ...
        <div className="flex flex-col gap-1.5">
          {(team.members || []).map((member, i) => (
            ...
            >
              ...
              <span className="text-sm font-semibold text-slate-800 font-[family-name:var(--font-inter)] leading-none flex-1 truncate">
                {member.name}
              </span>
          ...
          {Array.from({ length: Math.max(0, emptySlots) }).map((_, i) => (
            ...
            >
              ...
              <span className="text-xs text-slate-400 font-medium font-[family-name:var(--font-inter)]">
                Open slot
              </span>
            </div>
          ))}
      ...
      <div className="mt-auto w-full flex items-center justify-between gap-2 pt-3 border-t border-slate-100">
        ...
        <div className="flex items-center gap-2">
          {!isMyTeam ? (
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg border border-slate-200 bg-white text-slate-700 text-sm font-medium hover:bg-slate-50 hover:border-slate-300 transition-all disabled:opacity-40 disabled:cursor-not-allowed font-[family-name:var(--font-inter)]"
            >
            ...
            <>
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium transition-all font-[family-name:var(--font-inter)] ${
                  iAmReady
              ...
              <button
                onClick={() => onLeaveTeam()}
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-red-100 bg-red-50 text-red-600 text-sm font-medium hover:bg-red-100 hover:text-red-700 transition-all font-[family-name:var(--font-inter)]"
              >
        ...
        <span className="flex items-center gap-1.5 text-xs font-semibold text-slate-500 font-[family-name:var(--font-inter)]">
